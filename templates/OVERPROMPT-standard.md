...
2. **Build working queue:**
   ...
   - If all items are blocked, proceed to Phase 5 (retrospective) and exit

3. **Select next item:**
   - Choose highest priority item with `status: "ready"`
   - If no ready items exist, proceed to Phase 5 (retrospective) and exit
...
[Phase 1] INVOKE scan-prioritize-agent
  ...
  │
  └─→ IF no items in queue → [Phase 5] INVOKE retrospective-agent → END
  ↓
